...
# 该代码的核心思想是通过融合多种定位预测方法，优化建筑物内位置估计。主要包括以下方面：

# 预处理：读取Wi-Fi指纹、传感器距离、相对运动和绝对运动预测数据，以及其他辅助数据。这些数据将用于后续的优化过程。

# 配置：定义多个参数和权重，用于调整各种预测方法在优化过程中的贡献。例如，通过调整不同的距离惩罚、运动惩罚等因素，可以实现不同方法的平衡。

# 优化预测：该部分是算法的核心，它通过结合不同预测方法，生成最终的建筑物内位置估计。具体来说，它利用预处理阶段获取的数据，同时考虑配置参数，对位置进行权重优化。这一过程可以通过多进程进行加速。

# 结果输出：根据优化后的预测结果，计算最终的误差，并根据需要将结果保存为CSV文件。在验证模式下，还可以计算不同类型轨迹的误差。

# 通过将各种预测方法融合在一起，并根据场景和实际数据动态调整权重，这个代码的核心思想提供了一种有效地优化建筑物内位置估计的方法。